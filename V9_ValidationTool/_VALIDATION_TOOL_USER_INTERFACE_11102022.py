...
class App(ttk.Frame):    
    def __init__(self, master=None):
        # calling arcpy method so that the feature class selector loads faster later on
        # nothing = arcpy.Describe('C:\\')
        # nothing = None

        def startupThread():
            sT = Thread(target = emptyWork)
            sT.start()

        def emptyWork():
            arcpy.AddMessage('\n  Validating ArcGIS Licensing...')
            arcpy.AddMessage('\n')
            arcpy.AddMessage('  Initializing ArcPy\n')
    ...
    def isExitableState(self, exitableStatus, exitButton):# inputObjectMap):
        ...
        for inputVar in self.inputObjectMap:
            #print('current input variable: ', inputVar[0].get(), inputVar[1])   # #'1.0', 'end-1c'))   ### text widget???
            
            if (inputVar[0].get() == defaultInputComparator[inputVar[1]]) or (  (inputVar[1] == 'inFC' and arcpy.Exists(inputVar[0].get()) and arcpy.Describe(inputVar[0].get()).dataType != 'FeatureClass' ) or (inputVar[1] == 'inFC' and not arcpy.Exists(inputVar[0].get()) ))  or ( (inputVar[1] == 'outDir' and arcpy.Exists(inputVar[0].get()) and arcpy.Describe(inputVar[0].get()).dataType != 'Workspace') or (inputVar[1] == 'outDir' and not arcpy.Exists(inputVar[0].get()))):  
    ...
    def browseFor_inFC(self, full_path, childWin2):
        ...
        if path_name:
            desc =  arcpy.Describe(path_name) 
                  
            if desc.dataType == "Workspace":
                for dirpath, dirnames, filenames in arcpy.da.Walk(path_name, datatype="FeatureClass"):
                    for filename in filenames:
                        self.feature_classes.append(os.path.join(dirpath, filename))
                        self.files_list.append( str(filename ))
                #open a window with the list of feature classes 
    ...
    def create_widgets(self):
        ...
        def startScreenWidgetStateMutator():
            ...
            if (self.input_dict['isFinal'].get() == 'testModeSelected'):
                ...
                if (any([False if (self.input_dict[self.inputNameList[index]].get() == "") else True for index in indicesToCheck]) == False):
                    ...
                    moveToTest = tkMessageBox.askokcancel(title = "Proceed to Test Mode",
                                                          message = "You are about to leave Final mode and have input in other window(s). Continue?")
                    if moveToTest:
                        self.input_dict['isFinal'].set('testModeSelected')
        ...
        canvasDrawer = Canvas(buttonsFrame, height = 210, width = 15)
    ...
    def open_parcelDataInformationWindow(self):
        ...
        condoModel_input = ttk.Combobox(parcelDataInformationWindow, width=53, textvariable=condoModel_input_holder)   
            ...
            def isSaveableState(_):
                ...
                for inputVar in [(noticeOfNewStreetName_input, 'street'),
                    ...
                    if (inputVar[0].get('1.0', 'end-1c') == default_text[inputVar[1]]) or (inputVar[0].get('1.0', 'end-1c') == ""):
                        #print('comparison to defaults: ', inputVar[0].get('1.0', 'end-1c'), default_text[inputVar[1]])
    ...
    def open_plssLayerWindow(self):  
        ...
        def selectDataFormatStateMutator(_):
            ...
            elif (self.input_dict['PLSSType'].get() == 'Maintained by county feature class'):
        ...
        plssType_input = ttk.Combobox(plssLayerWindow, width = 53,
                                      values = ['Maintained by county feature class',
        ...
        def isPLSSReadytoRun(event ):
            ...
            if typeVar[0].get() == defaultInputComparator[typeVar[1]]: 
                ...
                if typeVar[0].get() == 'Maintained by county feature class':
                        ...
                        if (plssinputObject[0].get() == defaultInputComparator[plssinputObject[1]]) or \
                            (plssinputObject[1] == 'PLSSFC' and not arcpy.Exists(plssinputObject[0].get()) ) or \
                            (plssinputObject[1] == 'PLSSFC' and arcpy.Exists(plssinputObject[0].get()) and arcpy.Describe(plssinputObject[0].get()).dataType != 'FeatureClass' ) :
                ...
                if typeVar[0].get() ==   'Maintained by county as other digital format':
                        ...
                        if (plssinputObject[0].get() == defaultInputComparator[plssinputObject[1]]) or \
                            (plssinputObject[1] == 'PLSSOtherDigitalFile' and not os.path.isfile( plssinputObject[0].get()) ) :
                            unfilledFields.append(plssinputObject[2]) 
                        else:
                            if plssinputObject[2] in unfilledFields:
                                unfilledFields.remove(plssinputObject[2])                
    ...
    def open_zoningLayerWindow(self):  
        ...
        #combo box
        zoningGenType_input_label = Label(zoningLayerWindow, text='County General - with required DESCRIPTION/LINK Field:') 
        ...
        #combo box
        zoningShoreType_input_label = Label(zoningLayerWindow, text='Shoreland - with required DESCRIPTION/LINK Field:') 
        zoningShoreType_input_label.grid(row=5, column=0, padx=5, sticky='w')
        zoningShoreType_input = ttk.Combobox(zoningLayerWindow, width=53, textvariable=self.input_dict['zoningShoreType'])   
        ...
        #combo box
        zoningAirType_input_label = Label(zoningLayerWindow, text='Airport Protection - with required DESCRIPTION/LINK Field:') 
            ...
            for i in range(0,6,2):
                ...
                if typeVar[0].get() == defaultInputComparator[typeVar[1]]: 
                    ...
                    if typeVar[0].get() == 'Administered by county':
                        ...
                        if (fcVar[0].get() == defaultInputComparator[fcVar[1]]) or \
                             ((fcVar[1] == 'zoningGenFC' and   not arcpy.Exists(fcVar[0].get())) or (fcVar[1] == 'zoningGenFC' and   arcpy.Exists(fcVar[0].get()) and arcpy.Describe(fcVar[0].get()).dataType != 'FeatureClass' )) or\
                       	              ((fcVar[1] == 'zoningAirFC' and   not arcpy.Exists(fcVar[0].get())) or (fcVar[1] == 'zoningAirFC' and   arcpy.Exists(fcVar[0].get()) and arcpy.Describe(fcVar[0].get()).dataType != 'FeatureClass' )) or\
     								  ((fcVar[1] == 'zoningShoreFC' and not arcpy.Exists(fcVar[0].get())) or (fcVar[1] == 'zoningShoreFC' and arcpy.Exists(fcVar[0].get()) and arcpy.Describe(fcVar[0].get()).dataType != 'FeatureClass')): 
                            #print('adding')
    ...
    def open_otherLayersWindow(self):
        ...
        # scrollbar = Scrollbar(otherLayerWindow, orient = 'vertical')
        # scrollbar.grid(rowspan = 40, column = 2, sticky = 'NSE')
        # scrollbar.configure(command = otherLayerWindow.yview)
        # otherLayerWindow.configure(yscrollcommand = scrollbar.set)
        ...
        def hydrographyMutator(_):
            if (self.input_dict['HydroLineType'].get() == 'Maintained by county'):
                ...
                self.input_dict['HydroLineFC'].set('')
            ...
            for i in range(0,20,2):
                ...
                if typeVar[0].get() == defaultInputComparator[typeVar[1]]: 
                    ...
                    if typeVar[0].get() == 'Maintained by county':
                        ...
                        #print (fcVar[0].get() )
                        if (fcVar[0].get() == defaultInputComparator[fcVar[1]]) or ((arcpy.Exists(fcVar[0].get()) and arcpy.Describe(fcVar[0].get()).dataType != 'FeatureClass') or (not arcpy.Exists(fcVar[0].get()))) :
...
if __name__ == '__main__':  
    gui = Tix.Tk()
    gui.title('SCO Validation Tool v1.9') #title of main menu
    ...
    # GUI script first developed August 2022, by Branton Kunz, with support from Ana Wells and David Vogel
    # Modified by Param Bhandare
