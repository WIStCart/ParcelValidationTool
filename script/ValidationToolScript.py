...
inputNameList = ['isSearchable', 'isFinal', 'county', 'inFC', 'outDir', 'outName', 'outINIDir', 'subName', 'subEmail', 'condoModel', 'inCert', 'isNameRedact', 'redactPolicy', 'zoningGenType', 'zoningGenFC', 'zoningShoreType', 'zoningShoreFC', 'zoningAirType', 'zoningAirFC', 'PLSSType', 'PLSSFC', 'RightOfWayType', 'RightOfWayFC', 'RoadStreetCenterlineType',
                 'RoadStreetCenterlineFC', 'HydroLineType', 'HydroLineFC', 'HydroPolyType', 'HydroPolyFC', 'AddressesType', 'AddressesFC', 'BuildingBuildingFootprintType', 'BuildingBuildingFootprintFC', 'LandUseType', 'LandUseFC', 'ParksOpenSpaceType', 'ParksOpenSpaceFC', 'TrailsType', 'TrailsFC', 'OtherRecreationType', 'OtherRecreationFC', 'certifiedBy', 'PLSSOtherDigitalFile']
inputDict = collections.OrderedDict()
i = 0
for inputName in inputNameList:
    if inputName == 'isSearchable':
        # this parameter is no longer a user input because all data is searchable format
...
try:
    ...
    currVersion = urllib2.urlopen(
        'http://www.sco.wisc.edu/parcels/tools/Validation/validation_version.txt').read()
...
if inputDict['isSearchable'] == 'true':
    ...
    streetTypes = [line.strip() for line in open(os.path.join(
        base, '..\data\V6_StreetType_Simplified.txt'), 'r')]  # street types domain list
    unitIdTypes = [line.strip() for line in open(os.path.join(
        base, '..\data\V6_UnitId_Simplified.txt'), 'r')]  # unitid domain list
    unitTypes = [line.strip() for line in open(os.path.join(
        base, '..\data\V6_UnitType_Simplified.txt'), 'r')]  # unit type domain list
    lsadDomains = [line.strip() for line in open(os.path.join(
        base, '..\data\LSAD_Simplified.txt'), 'r')]  # lsad domain list
    # taxroll years to test (past,expected,future1,future2)
    taxRollYears = [line.strip() for line in open(
        os.path.join(base, '..\data\V6_TaxRollYears.txt'), 'r')]
    suffixDomains = [line.strip() for line in open(os.path.join(
        base, '..\data\V6_SuffixDomains_Simplified.txt'), 'r')]  # suffix domain list
    prefixDomains = [line.strip() for line in open(os.path.join(
        base, '..\data\V6_PrefixDomains_Simplified.txt'), 'r')]  # prefix domain list
    # list of non-parcelid values found in field to ignore when checking for dups (and use in other functions)
    pinSkips = [line.strip() for line in open(
        os.path.join(base, '..\data\V6_PinSkips.txt'), 'r')]

    # school district code list (csv has been updated for V5/2018 school districts)
    reader = csv.reader(
        open(os.path.join(base, '..\data\school_district_codes.csv'), 'rU'))
    ...
    # CONAME and FIPS list
    reader = csv.reader(
        open(os.path.join(base, '..\data\CoNameFips.csv'), 'rU'))
    county_nameNo_dict = {}
    county_noName_dict = {}
    for row in reader:
        k, v = row
    ...
    arcpy.FeatureClassToFeatureClass_conversion(
        inputDict['inFC'], dynamic_workspace, "WORKING")
    ...
    arcpy.AddField_management(
        output_fc_temp, "GeneralElementErrors", "TEXT", "", "", 1000)
    arcpy.AddField_management(
        output_fc_temp, "AddressElementErrors", "TEXT", "", "", 1000)
    arcpy.AddField_management(
        output_fc_temp, "TaxrollElementErrors", "TEXT", "", "", 1000)
    arcpy.AddField_management(
        output_fc_temp, "GeometricElementErrors", "TEXT", "", "", 1000)
    ...
    with arcpy.da.UpdateCursor(output_fc_temp, fieldNames) as cursor:
        for row in cursor:
            ...
            if totError.badcharsCount > 100:
                totError = Error.checkBadChars(
                    totError)  # for wrong <null> values
            ...
            totError, currParcel = Error.checkNumericTextValue(
                totError, currParcel, "estfmkvalue", "tax", True)
            totError, currParcel = Error.checkNumericTextValue(
                totError, currParcel, "netprpta", "tax", True)
            totError, currParcel = Error.checkNumericTextValue(
                totError, currParcel, "grsprpta", "tax", True)
            totError, currParcel = Error.checkNumericTextValue(
                totError, currParcel, "assdacres", "tax", True)
            totError, currParcel = Error.checkNumericTextValue(
                totError, currParcel, "deedacres", "tax", True)
            totError, currParcel = Error.checkNumericTextValue(
                totError, currParcel, "gisacres", "tax", True)
            totError, currParcel = Error.checkIsDuplicate(
                totError, currParcel, "parcelid", "general", True, pinSkips, uniquePinList)
            totError, currParcel = Error.checkIsDuplicate(
                totError, currParcel, "taxparcelid", "general", True, pinSkips, uniqueTaxparList)
            totError, currParcel = Error.checkDomainString(
                totError, currParcel, "prefix", "address", True, prefixDomains)
            totError, currParcel = Error.checkDomainString(
                totError, currParcel, "streettype", "address", True, streetTypes)
            totError, currParcel = Error.checkDomainString(
                totError, currParcel, "unittype", "address", True, unitTypes)
            totError, currParcel = Error.checkDomainString(
                totError, currParcel, "unitid", "address", True, unitIdTypes)
            totError, currParcel = Error.checkDomainString(
                totError, currParcel, "placename", "general", True, lsadDomains)
            totError, currParcel = Error.checkDomainString(
                totError, currParcel, "suffix", "address", True, suffixDomains)
#totError,currParcel = Error.badChars(totError,currParcel,fieldNames,fieldNamesBadChars,'general')
            totError, currParcel = Error.trYear(
                totError, currParcel, "taxrollyear", "parcelid", "tax", False, pinSkips, taxRollYears)
            totError, currParcel = Error.taxrollYrCheck(
                totError, currParcel, "taxrollyear", "tax", False, pinSkips, taxRollYears)
            totError, currParcel = Error.streetNameCheck(
                totError, currParcel, "streetname", "siteadress", "address", True, stNameDict, inputDict['county'])
            totError, currParcel = Error.zipCheck(
                totError, currParcel, "zipcode", "address", True)
            totError, currParcel = Error.zip4Check(
                totError, currParcel, "zip4", "address", True)
            #totError,currParcel = Error.impCheck(totError,currParcel,"improved","impvalue","tax")

            totError, currParcel = Error.totCheck(
                totError, currParcel, "impvalue", "cntassdvalue", "lndvalue", "tax")

            # EXAMPLE FUNCTION # totError,currParcel = Error.reallyBadChars(totError,currParcel,fieldNames,fieldNamesBadChars,'general')
            totError, currParcel = Error.checkRedundantID(
                totError, currParcel, 'taxparcelid', 'parcelid', True, 'general')
            totError, currParcel = Error.postalCheck(
                totError, currParcel, 'pstladress', 'general', pinSkips, 'taxrollyear', 'parcelid', badPstladdSet, taxRollYears)
            totError, currParcel = Error.auxPropCheck(
                totError, currParcel, 'propclass', 'auxclass', 'taxrollyear', 'parcelid', pinSkips, 'tax', copDomains, auxDomains, taxRollYears)
            #arcpy.AddMessage('On record:'+ str(currParcel.objectid))
            totError, currParcel = Error.totalAssdValueCheck(
                totError, currParcel, 'cntassdvalue', 'lndvalue', 'impvalue', 'tax')
            totError, currParcel = Error.fairMarketCheck(
                totError, currParcel, 'propclass', 'auxclass', 'estfmkvalue', 'tax')
            totError, currParcel = Error.mfLValueCheck(
                totError, currParcel, 'mflvalue', 'auxclass', 'tax')
            totError, currParcel = Error.mflLndValueCheck(
                totError, currParcel, "parcelid", parcelidList, "lndvalue", "mflvalue", "tax")
            totError, currParcel = Error.auxclassFullyX4Check(
                totError, currParcel, 'auxclass', 'propclass', 'tax')
            #totError,currParcel = Error.auxclassTaxrollCheck (totError,currParcel,'auxclass', 'tax')
            totError, currParcel = Error.matchContrib(
                totError, currParcel, "coname", "parcelfips", "parcelsrc", county_nameNo_dict, county_noName_dict, False, "general")
            totError, currParcel = Error.netVsGross(
                totError, currParcel, "netprpta", "grsprpta", "tax")
            totError, currParcel = Error.schoolDistCheck(
                totError, currParcel, "parcelid", "schooldist", "schooldistno", schoolDist_noName_dict, schoolDist_nameNo_dict, "tax", pinSkips, "taxrollyear")
            totError, currParcel = Error.propClassCntCheck(
                totError, currParcel, "propclass", "cntassdvalue", "tax")

            totError, currParcel = Error.fieldCompleteness(
                totError, currParcel, fieldNames, fieldListPass, CompDict)
    ...
    totError = Error.fieldCompletenessComparison(totError, fieldNames, fieldListPass, CompDict, getattr(
        LegacyCountyStats, (inputDict['county'].replace(" ", "_").replace(".", ""))+"LegacyDict"))
    ...
    totError.ErrorSum = totError.generalErrorCount + totError.geometricErrorCount + \
        totError.addressErrorCount + totError.taxErrorCount
    ...
    if totError.geometryNotValidated == True:
        arcpy.AddMessage(
            "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
        arcpy.AddMessage(
            "THE GEOMETRY OF THIS FEATURE CLASS WAS NOT VALIDATED  \n")
        arcpy.AddMessage("THE FEATURE CLASS IS ABOUT " + str(totError.xyShift) +
                         " METERS DISPLACED FROM DATA SUBMITTED LAST YEAR. \n")
        arcpy.AddMessage(
            "THIS ISSUE IS INDICATIVE OF A RE-PROJECTION ERROR. \n ")
        arcpy.AddMessage(
            "PLEASE MAKE NEEDED ALTERATIONS TO THE FEATURE CLASS AND RUN THE TOOL AGAIN.\n")
        arcpy.AddMessage(
            "CHECK THE DOCUMENTATION: http://www.sco.wisc.edu/parcels/tools/FieldMapping/Parcel_Schema_Field_Mapping_Guide.pdf Section 2 \n")
        arcpy.AddMessage(
            "CONTACT THE PARCEL TEAM AT SCO WITH QUESTIONS ABOUT THIS ISSUE.\n")
        arcpy.AddMessage(
            "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
        sys.exit()
    ...
    if inputDict['isFinal'] == 'false':
        ...
        outSummaryJSON = os.path.join(base, '..\summary\summary.js')
        # full (hard coded) path to the Validation Summary Page (escape \v with a \\)
        outSummaryPage = os.path.join(base, '..\summary\\validation.html')
        # full (hard coded) path to the Validation Summary directory
        outSummaryDir = os.path.join(base, '..\summary')
        Summary.writeSummaryTxt(
            summary, outSummaryDir, inputDict['outName'], totError, outSummaryPage, outSummaryJSON)

        # Write feature class from memory back out to hard disk
        arcpy.FeatureClassToFeatureClass_conversion(
            output_fc_temp, inputDict['outDir'], inputDict['outName'])
        arcpy.AddMessage(
            "\n\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
        arcpy.AddMessage("TEST RUN COMPLETE\n")
        arcpy.AddMessage("REVIEW THE VALIDATION SUMMARY PAGE (" + outSummaryPage.replace(
            "\script\..", "") + ") FOR A SUMMARY OF THE POTENTIAL ISSUES FOUND.\n")
        arcpy.AddMessage(
